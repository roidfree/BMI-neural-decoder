...
% Rebin
    processed_data = rebin_data(processed_data, trials, movements, neurons, config.bin_width);

% Transform
    processed_data = transform_data(processed_data, trials, movements, neurons, config.transform);
...
% Convolution (optional)
    ...
    for m = 1:movements
        ...
        for t = 1:trials
            for i = 1:max_iter
                X(m, counter, :) = reshape(processed_data(t, m).spikes(:, i:history_bins + i), 1, []);
                Y(m, counter, 1) = (processed_data(t, m).handPos(1, (history_bins + i) * bin_width) - processed_data(t, m).handPos(1, (history_bins + i - 1) * bin_width)) - (modelParameters.avgTraj{m}(1, (history_bins + i) * bin_width) - modelParameters.avgTraj{m}(1, (history_bins + i - 1) * bin_width));
                Y(m, counter, 2) = (processed_data(t, m).handPos(2, (history_bins + i) * bin_width) - processed_data(t, m).handPos(2, (history_bins + i - 1) * bin_width)) - (modelParameters.avgTraj{m}(2, (history_bins + i) * bin_width)- modelParameters.avgTraj{m}(2, (history_bins + i - 1) * bin_width));
                counter = counter + 1;
            end
        end
    end
    ...
    for m = 1:movements
        ...
        centred_X = Xmov - mu_X;
        [U, S, V] = svd(centred_X);
        if isfield(config, 'pca_dim')
        PCs = min(config.pca_dim, size(U,2));
            else
        PCs = min(50, size(U,2));  % default
            end
        U_reduced = U(:, 1:PCs);
        S_reduced = S(1:PCs, 1:PCs);
        V_reduced = V(:, 1:PCs);  % these are eigenvectors of covariance matrix
